...
const runId = Number(String(Date.now()).slice(-8)) * 10 + Math.floor(Math.random() * 10);
let seasonA;
let seasonB;
let userA;
let userB;
...
try {
  ...
  const objects = await db.query(
    `SELECT table_name FROM information_schema.tables
      ...
      ORDER BY table_name`,
    [["users", "user_state", "admins", "seasons", "prizes", "spins", "payouts", "daily_gift_claims", "owner_gifts", "channel_activity", "audit_logs", "star_transactions", "stars_ledger"]],
  );
  ...
  const users = await db.query(
    `INSERT INTO users (telegram_id,username,first_name)
     VALUES ($1,$4,'A'),($2,$5,'B'),($3,$6,'C')
     RETURNING id,username`,
    [910000000 + runId * 3 + 1, 910000000 + runId * 3 + 2, 910000000 + runId * 3 + 3, `ci_a_${suffix}`, `ci_b_${suffix}`, `ci_c_${suffix}`],
  );
  userA = users.rows.find((r) => r.username === `ci_a_${suffix}`).id;
  userB = users.rows.find((r) => r.username === `ci_b_${suffix}`).id;
  userC = users.rows.find((r) => r.username === `ci_c_${suffix}`).id;
  ...
  const starsPrize = await db.query(
    `INSERT INTO prizes (season_id,kind,title,amount,quantity_total,quantity_remaining,is_active,metadata)
     VALUES ($1,'STARS','Custom 137 Stars',137,3,3,TRUE,$2) RETURNING id,amount`,
  ...
  const inactivePrize = await db.query(
    `INSERT INTO prizes (season_id,kind,title,amount,quantity_total,quantity_remaining,is_active)
     VALUES ($1,'MONEY','Inactive',999,20,20,FALSE) RETURNING id`,
    [seasonA],
  );
  ...
  await db.query(
    `INSERT INTO stars_ledger (user_id,season_id,type,amount,idempotency_key,metadata)
     VALUES ($1,$2,'REWARD',$3,$4,$5::jsonb)`,
    [userA, seasonA, requestedReward, `ci-reward:${suffix}`, JSON.stringify({ requestedAmount: requestedReward, creditedAmount: credited, overflowAmount: overflow })],
  );
  await db.query(`UPDATE user_state SET stars_balance=stars_balance+$2 WHERE user_id=$1`, [userA, credited]);
  if (overflow > 0) {
    await db.query(
      `INSERT INTO stars_ledger (user_id,season_id,type,amount,idempotency_key,metadata)
       VALUES ($1,$2,'CAPPED_OVERFLOW_BURNED',$3,$4,$5::jsonb)`,
      [userA, seasonA, -overflow, `ci-overflow:${suffix}`, JSON.stringify({ requestedAmount: requestedReward, creditedAmount: credited, overflowAmount: overflow })],
    );
  }
  ...
  await expectReject(
    () => db.query(`INSERT INTO star_transactions (user_id,amount,status,payload) VALUES ($1,100,'PENDING',$2)`, [userA, JSON.stringify({ payload: `paidspin:v1:${userA}:${seasonA}:ci-${suffix}-duplicate`, type: "PAID_SPIN", seasonId: seasonA, userId: userA })]),
