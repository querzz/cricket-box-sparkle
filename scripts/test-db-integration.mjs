...
try {
  ...
  // Schema smoke test: required production objects are present.
  const objects = await db.query(`
    SELECT table_name FROM information_schema.tables
    WHERE table_schema='public' AND table_name = ANY($1::text[])
    ORDER BY table_name
  `, [["users", "user_state", "admins", "seasons", "prizes", "spins", "payouts", "daily_gift_claims", "owner_gifts", "channel_activity", "audit_logs", "star_transactions"]]);
  assert(objects.rowCount === 12, "all core tables exist");
  ...
  const adminResult = await db.query(`
    ...
    RETURNING id
  `, [900000000 + Number(String(suffix).replace(/\D/g, "").slice(-5) || 1), `ci_${suffix}`]);
  ...
  seasonA = seasons.rows.find((r) => r.code.endsWith("-A")).id;
  seasonB = seasons.rows.find((r) => r.code.endsWith("-B")).id;

  const users = await db.query(`
    INSERT INTO users (telegram_id, username, first_name)
    VALUES ($1, 'ci_a', 'A'), ($2, 'ci_b', 'B'), ($3, 'ci_c', 'C')
    RETURNING id, username
  `, [910000001, 910000002, 910000003]);
  userA = users.rows.find((r) => r.username === "ci_a").id;
  userB = users.rows.find((r) => r.username === "ci_b").id;
  userC = users.rows.find((r) => r.username === "ci_c").id;

  await db.query(`INSERT INTO user_state (user_id, stars_balance) VALUES ($1, 480), ($2, 500), ($3, 100)`, [userA, userB, userC]);

  // Prize Builder contract: arbitrary Money/Stars amounts, quantity, weight, active, image.
  const prize = await db.query(`
    ...
    VALUES ($1, 'MONEY', 'Custom 1375 UAH', 'CI test', 1375, 1000, 'UAH', 7, 7, TRUE, 'https://example.com/prize.png', $2)
    RETURNING id, amount, quantity_total, quantity_remaining, is_active, metadata
  `, [seasonA, JSON.stringify({ weight: 2.5 })]);
  ...
  const starsPrize = await db.query(`
    INSERT INTO prizes (season_id, kind, title, amount, quantity_total, quantity_remaining, is_active, metadata)
    VALUES ($1, 'STARS', 'Custom 137 Stars', 137, 3, 3, TRUE, $2)
    RETURNING id, amount
  `, [seasonA, JSON.stringify({ weight: 4 })]);
  assert(Number(starsPrize.rows[0].amount) === 137, "arbitrary Stars amount persists");

  const inactivePrize = await db.query(`
    INSERT INTO prizes (season_id, kind, title, amount, quantity_total, quantity_remaining, is_active)
    VALUES ($1, 'MONEY', 'Inactive', 999, 20, 20, FALSE)
    RETURNING id
  `, [seasonA]);
  ...
  // Stars cap DB invariant.
  await expectReject(
    () => db.query(`UPDATE user_state SET stars_balance = 501 WHERE user_id=$1`, [userA]),
    "Stars balance above 500 is rejected",
  );

  // Successful reward credit must never exceed cap.
  ...
  assert(requestedReward - credited === 30, "overflow can be represented deterministically");

  // Payment idempotency DB guard.
  ...
  `, [userA, JSON.stringify({ payload, type: "PAID_SPIN", seasonId: seasonA, userId: userA })]);
  await expectReject(
    () => db.query(`
      INSERT INTO star_transactions (user_id, amount, status, payload)
      VALUES ($1, 100, 'PENDING', $2)
    `, [userA, JSON.stringify({ payload: `paidspin:v1:${userA}:${seasonA}:ci-${suffix}-duplicate`, type: "PAID_SPIN", seasonId: seasonA, userId: userA })]),
    "one pending paid spin per user and season is enforced",
  );

  // Idempotency is released after the original transaction is no longer pending.
  await db.query(`UPDATE star_transactions SET status='SUCCESS', processed_at=now() WHERE user_id=$1 AND status='PENDING'`, [userA]);
  await db.query(`
    INSERT INTO star_transactions (user_id, amount, status, payload)
    VALUES ($1, 100, 'PENDING', $2)
  `, [userA, JSON.stringify({ payload: `paidspin:v1:${userA}:${seasonA}:ci-${suffix}-2`, type: "PAID_SPIN", seasonId: seasonA, userId: userA })]);

  // Leaderboard is season-scoped and uses real users/spins/payouts.
  ...
  `, [userA, userB, seasonA]);
  await db.query(`
    INSERT INTO payouts (spin_id, user_id, prize_id, kind, amount, currency, status)
    VALUES ($1,$2,$3,'STARS',137,'XTR','PAID')
  `, [spin1.rows[0].id, userA, starsPrize.rows[0].id]);
  ...
  assert(board.rowCount === 2, "leaderboard contains only users with completed spins in the season");
  ...
  // Transactional inventory race: only one concurrent claim may consume a single unit.
  const racePrize = await db.query(`
    INSERT INTO prizes (season_id, kind, title, amount, quantity_total, quantity_remaining, is_active)
    VALUES ($1,'MONEY','Race Prize',10,1,1,TRUE) RETURNING id
  `, [seasonA]);
  const claim = async () => {
    ...
    try {
      await c.query('BEGIN');
      const locked = await c.query(`SELECT quantity_remaining FROM prizes WHERE id=$1 FOR UPDATE`, [racePrize.rows[0].id]);
      let won = false;
      if (Number(locked.rows[0]?.quantity_remaining ?? 0) > 0) {
        await c.query(`UPDATE prizes SET quantity_remaining=quantity_remaining-1 WHERE id=$1`, [racePrize.rows[0].id]);
        won = true;
      }
  ...
  if (admin || seasonA || seasonB || userA || userB || userC) {
    await db.query(`DELETE FROM star_transactions WHERE user_id = ANY($1::uuid[])`, [[userA, userB, userC].filter(Boolean)]);
    await db.query(`DELETE FROM payouts WHERE user_id = ANY($1::uuid[])`, [[userA, userB, userC].filter(Boolean)]);
    await db.query(`DELETE FROM spins WHERE user_id = ANY($1::uuid[])`, [[userA, userB, userC].filter(Boolean)]);
    await db.query(`DELETE FROM prizes WHERE season_id = ANY($1::uuid[])`, [[seasonA, seasonB].filter(Boolean)]);
    await db.query(`DELETE FROM user_state WHERE user_id = ANY($1::uuid[])`, [[userA, userB, userC].filter(Boolean)]);
    await db.query(`DELETE FROM users WHERE id = ANY($1::uuid[])`, [[userA, userB, userC].filter(Boolean)]);
    await db.query(`DELETE FROM seasons WHERE id = ANY($1::uuid[])`, [[seasonA, seasonB].filter(Boolean)]);
