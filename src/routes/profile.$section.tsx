...
type SupportInfo = { support: SupportContact | null };
...
function SectionScreen() {
  ...
  const [support, setSupport] = useState<SupportContact | undefined>();
  ...
  useEffect(() => {
    ...
    const loadSupport = async () => {
      try {
        ...
        const data = (await response.json()) as SupportInfo;
  ...
  return (
    <AppShell title={title} back="/profile">
      {section === "leaderboard" && <div className="space-y-2.5"><GlassCard className="px-4 py-3"><p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Текущий сезон</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Рейтинг строится по активности сезона: сначала количество завершённых прокруток, затем победы и Stars, выигранные в этом сезоне.</p></GlassCard>{leaderboard.length > 0 ? <GlassCard className="divide-y divide-glass-border overflow-hidden">{leaderboard.map((leader) => <div key={leader.userId} className={`flex items-center gap-3 px-4 py-3.5 ${leader.isCurrentUser ? "bg-primary/8" : ""}`}><span className="grid size-7 shrink-0 place-items-center rounded-lg bg-muted/20 text-xs font-semibold text-primary">{leader.rank <= 3 ? <Medal className="size-3.5" /> : leader.rank}</span><div className="grid size-9 shrink-0 place-items-center overflow-hidden rounded-full border border-glass-border bg-muted/20"><span className="text-xs font-semibold text-muted-foreground">{leader.username.replace("@", "").slice(0, 1).toUpperCase()}</span></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{leader.username}{leader.isCurrentUser ? " · ты" : ""}</p><p className="text-[10px] text-muted-foreground">{leader.spins} прокруток · {leader.wins} побед · {leader.level} ур.</p></div><span className="shrink-0 font-display text-sm tabular-nums">{leader.starsWon} ⭐</span></div>)}</GlassCard> : <EmptyState title="Лидеров пока нет" description="Сделай первую прокрутку — рейтинг появится здесь автоматически." />}<div className="flex items-center gap-2 px-1 text-[10px] text-muted-foreground"><Trophy className="size-3.5 text-primary" /> Рейтинг обновляется из PostgreSQL после завершённых действий.</div></div>}
      ...
      {section === "support" && <GlassCard className="space-y-3 px-4 py-5 text-center"><p className="font-display text-sm uppercase tracking-[0.16em]">Нужна помощь?</p><p className="text-xs leading-relaxed text-muted-foreground">Напишите в поддержку CRICKET BOX. Никому не сообщайте платёжные данные.</p>{support?.url ? <a href={support.url} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl border border-primary/40 bg-primary/10 px-4 py-2.5 text-sm font-semibold text-primary-glow">{support.username ? `@${support.username.replace(/^@/, "")}` : "Открыть поддержку"}<ExternalLink className="size-4" /></a> : <p className="text-[11px] text-muted-foreground">Контакт поддержки пока недоступен.</p>}</GlassCard>}
