...
const rules = [
  ...
  "Дополнительные прокрутки можно покупать за Stars Cricket Box.",
...
const faq = [
  { q: "Stars Cricket Box — это те же Telegram Stars?", a: "Нет. Stars внутри Cricket Box используются только в механике этого приложения и отделены от личного баланса Telegram Stars." },
  ...
  { q: "Когда я получу свой приз?", a: "Цифровые и другие призы обрабатываются администратором после завершения сезона и проверки результата." },
];
...
function SectionScreen() {
  ...
  return (
    <AppShell title={title} back="/profile">
      ...
      {section === "history" && <div className="space-y-2.5">{snapshot && snapshot.rewards.length > 0 ? snapshot.rewards.map((reward) => <GlassCard key={reward.id} className="flex items-center gap-3 px-4 py-3"><div className="min-w-0 flex-1"><p className="truncate text-sm">{reward.title}</p><p className="text-[11px] text-muted-foreground">{formatDate(reward.wonAt)}</p></div><StatusBadge status={{ type: "reward", value: reward.status }} /></GlassCard>) : <EmptyState title="Активность пока отсутствует" />}</div>}
