      ...
      POST: async ({ request }) => {
        try {
          ...
          const result = await withTransaction(async (client) => {
            ...
            const recent = await client.query<{kind:string}>(`SELECT p.kind FROM spins s LEFT JOIN prizes p ON p.id=s.prize_id WHERE s.user_id=$1::uuid AND s.season_id=$2::uuid AND s.status='COMPLETED' ORDER BY s.created_at DESC LIMIT 20`,[user.rows[0].id,current.id]);
