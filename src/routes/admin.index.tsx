...
function AdminDashboard() {
  ...
  function updatePremiumCost(id: string, value: number) {
    setPrizes((all) => all.map((prize) => prize.id === id ? { ...prize, unitCost: Math.max(0, value) } : prize));
  }

  function addPrize() {
    setPrizes((all) => [...all, { id: `custom_${Date.now()}`, kind: "STARS", title: "Новая награда", subtitle: "Telegram Stars", amount: 10, quantity: 1, unitCost: 0 }]);
  }
