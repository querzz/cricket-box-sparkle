...
function ActionLink({ icon: Icon, title, text, href }: { icon: ComponentType<{ className?: string }>; title: string; text: string; href: string }) { return <Link to={href} aria-label={title} className="press relative z-20 isolate block h-full w-full text-left"><GlassCard className="pointer-events-none h-full px-3 py-3.5"><Icon className="size-4 text-primary-glow" /><p className="mt-2 text-sm font-semibold">{title}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{text}</p></GlassCard></Link>; }
