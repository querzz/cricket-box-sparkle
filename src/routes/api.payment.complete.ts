...
export const Route=createFileRoute("/api/payment/complete")({server:{handlers:{POST:async({request})=>{
  try{
    ...
    const userId=parts[2]; const seasonId=parts[3];
    ...
    const result=await withTransaction(async client=>{
      ...
      const seasonResult=await client.query<{state:string;starts_at:string|null;ends_at:string|null;paid_spin_enabled:boolean}>(`SELECT state,starts_at::text,ends_at::text,paid_spin_enabled FROM seasons WHERE id=$1::uuid FOR UPDATE`,[seasonId]);
      const season=seasonResult.rows[0];
      if(!season||!["ACTIVE","ENDING"].includes(season.state))throw new Error("SEASON_NOT_ACTIVE");
      if(!season.paid_spin_enabled)throw new Error("PAID_SPIN_DISABLED");
      if(Number(tx.rows[0].amount)!==totalAmount)throw new Error("PAYMENT_AMOUNT_MISMATCH");

      const recent=await client.query<{kind:string}>(`SELECT p.kind FROM spins s LEFT JOIN prizes p ON p.id=s.prize_id WHERE s.user_id=$1::uuid AND s.season_id=$2::uuid AND s.status='COMPLETED' ORDER BY s.created_at DESC LIMIT 20`,[userId,seasonId]);
      ...
      const economyPrizes=prizes.rows.map(prize=>({
        ...prize,
        metadata:{...(prize.metadata??{}),economyMultiplier:getEconomyMultiplier({quantityTotal:prize.quantity_total,quantityRemaining:prize.quantity_remaining,elapsedFraction})},
      }));
      ...
      if(rewardStars>0){
        await appendStarsLedger(client,{userId,seasonId,spinId:spin.rows[0].id,type:"REWARD",amount:rewardStars,balanceDelta:credited,referenceId:picked.id,idempotencyKey:`paid-spin:${spin.rows[0].id}:stars-reward`,metadata:{requestedAmount:rewardStars,creditedAmount:credited,overflowAmount:overflow,source:"PAID_SPIN"}});
