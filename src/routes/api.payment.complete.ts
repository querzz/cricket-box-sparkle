...
export const Route=createFileRoute("/api/payment/complete")({server:{handlers:{POST:async({request})=>{
  ...
  try{
    ...
    const result=await withTransaction(async client=>{
      ...
      const rewardStars=picked.kind==="STARS"?Math.max(0,Math.floor(Number(picked.amount)||0)):0; const credited=rewardStars>0?Math.min(rewardStars,Math.max(0,STARS_MAX_BALANCE-Number(state.stars_balance??0))):0; const overflow=Math.max(0,rewardStars-credited);
