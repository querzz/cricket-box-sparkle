...
export const Route=createFileRoute("/api/payment/complete")({server:{handlers:{POST:async({request})=>{
  try{
    ...
    const result=await withTransaction(async client=>{
      ...
      const recent=await client.query<{kind:PrizeRow["kind"]}>(`SELECT p.kind FROM spins s JOIN prizes p ON p.id=s.prize_id WHERE s.user_id=$1::uuid AND s.season_id=$2::uuid AND s.status='COMPLETED' ORDER BY s.created_at DESC LIMIT 30`,[userId,seasonId]);
      const recentKinds=recent.rows.map(row=>row.kind);
      let emptyStreak=0; for(const kind of recentKinds){if(kind!=="EMPTY")break;emptyStreak+=1;}
      const elapsedFraction=seasonElapsedFraction(season.starts_at,season.ends_at);
      const prizes=await client.query<PrizeRow>(`SELECT id::text,kind,title,subtitle,amount::text,currency,quantity_total,quantity_remaining,metadata FROM prizes WHERE season_id=$1::uuid AND quantity_remaining>0 AND is_active=TRUE AND (kind<>'STARS' OR $2::integer<$3::integer) ORDER BY created_at ASC FOR UPDATE`,[seasonId,Number(state.stars_balance??0),MAX_STARS]); if(!prizes.rows.length)throw new Error("NO_PRIZES");
      const selection=pickDynamicPrize(prizes.rows,secureRandomUnit,{elapsedFraction,emptyStreak,recentKinds}); const picked=selection.prize; const inventory=await client.query(`UPDATE prizes SET quantity_remaining=quantity_remaining-1,updated_at=now() WHERE id=$1::uuid AND quantity_remaining>0 RETURNING id`,[picked.id]); if(!inventory.rows[0])throw new Error("NO_PRIZES");
      ...
      if(rewardStars>0){await appendStarsLedger(client,{userId,seasonId,spinId:spin.rows[0].id,type:"REWARD",amount:rewardStars,balanceDelta:credited,referenceId:picked.id,idempotencyKey:`paid-spin:${spin.rows[0].id}:stars-reward`,metadata:{requestedAmount:rewardStars,creditedAmount:credited,overflowAmount:overflow,source:"PAID_SPIN"}}); if(overflow>0)await appendStarsLedger(client,{userId,seasonId,spinId:spin.rows[0].id,type:"CAPPED_OVERFLOW_BURNED",amount:-overflow,balanceDelta:0,referenceId:picked.id,idempotencyKey:`paid-spin:${spin.rows[0].id}:stars-overflow`,metadata:{requestedAmount:rewardStars,creditedAmount:credited,overflowAmount:overflow,source:"PAID_SPIN"}});}
      let payoutId:string|null=null; if(picked.kind!=="EMPTY"){const payoutStatus=rewardStars>0?"PAID":"PENDING"; const note=rewardStars>0?(credited<rewardStars?`Лимит 500 Stars: зачислено ${credited} из ${rewardStars}.`:"Stars зачислены на баланс."):"Награда ожидает выдачи администратором."; const payout=await client.query<{id:string}>(`INSERT INTO payouts(spin_id,user_id,prize_id,kind,amount,currency,status,note,paid_at) VALUES($1::uuid,$2::uuid,$3::uuid,$4,$5::numeric,$6,$7,$8,CASE WHEN $7='PAID' THEN now() ELSE NULL END) RETURNING id::text`,[spin.rows[0].id,userId,picked.id,picked.kind,picked.amount,picked.currency,payoutStatus,note]); payoutId=payout.rows[0].id;}
      const nextXp=Number(user.rows[0].xp??0)+10; await client.query(`UPDATE users SET xp=$2,level=$3,last_seen_at=now() WHERE id=$1::uuid`,[userId,nextXp,Math.max(1,Math.floor(nextXp/100)+1)]); await client.query(`UPDATE star_transactions SET status='SUCCESS',telegram_charge_id=$2,spin_id=$3::uuid,processed_at=now(),payload=payload||$4::jsonb WHERE id=$1::uuid`,[tx.rows[0].id,chargeId,spin.rows[0].id,JSON.stringify({telegramId,currency,totalAmount,creditedStars:credited,overflowStars:overflow})]);
      await client.query(`INSERT INTO audit_logs(action,entity_type,entity_id,after_data) VALUES('PAID_SPIN_COMPLETED','spin',$1,$2::jsonb)`,[spin.rows[0].id,JSON.stringify({userId,seasonId,prizeId:picked.id,chargeId,totalAmount,payoutId,rewardKind:picked.kind,creditedStars:credited,overflowStars:overflow,algorithmVersion:"dynamic-v1",elapsedFraction,emptyStreak,recentKinds:recentKinds.slice(0,8),selection:selection.diagnostics[picked.id]})]);
  ...
  }catch(error){const code=error instanceof Error?error.message:"PAYMENT_COMPLETE_FAILED"; const status=["PAYMENT_NOT_FOUND","PAYMENT_USER_MISMATCH","USER_NOT_FOUND"].includes(code)?404:["SEASON_NOT_ACTIVE","NO_PRIZES","PAYMENT_NOT_PENDING","PAID_SPIN_DISABLED"].includes(code)?409:400; return Response.json({ok:false,code},{status});}
}}}});
