      ...
      POST: async ({ request }) => {
        try {
          ...
          const result = await withTransaction(async (client) => {
            ...
            const spin = await client.query<{ id: string; created_at: string }>(`INSERT INTO spins (user_id, season_id, type, price_stars, prize_id, status, completed_at) VALUES ($1::uuid, $2::uuid, $3, 0, $4::uuid, 'COMPLETED', now()) RETURNING id::text, created_at::text`, [user.rows[0].id, currentSeason.id, useBonus ? "ACTIVITY_BONUS" : "FREE", picked.id]);
            ...
            if (rewardStars > 0) {
              await appendStarsLedger(client, {
                userId: user.rows[0].id, seasonId: currentSeason.id, spinId: spin.rows[0].id, type: "REWARD", amount: rewardStars, balanceDelta: credited,
                referenceId: picked.id, idempotencyKey: `spin:${spin.rows[0].id}:stars-reward`, metadata: { requestedAmount: rewardStars, creditedAmount: credited, overflowAmount: overflow, source: useBonus ? "ACTIVITY_BONUS" : "FREE_SPIN" },
              });
              if (overflow > 0) await appendStarsLedger(client, {
                userId: user.rows[0].id, seasonId: currentSeason.id, spinId: spin.rows[0].id, type: "CAPPED_OVERFLOW_BURNED", amount: -overflow, balanceDelta: 0,
                referenceId: picked.id, idempotencyKey: `spin:${spin.rows[0].id}:stars-overflow`, metadata: { requestedAmount: rewardStars, creditedAmount: credited, overflowAmount: overflow },
              });
            }
            ...
            await client.query(`INSERT INTO audit_logs (action, entity_type, entity_id, after_data) VALUES ('SPIN_COMPLETED', 'spin', $1, $2::jsonb)`, [spin.rows[0].id, JSON.stringify({ userId: user.rows[0].id, seasonId: currentSeason.id, prizeId: picked.id, type: useBonus ? "ACTIVITY_BONUS" : "FREE", usedDaily: useDaily, usedBonus: useBonus, rewardKind: picked.kind, creditedStars: credited, overflowStars: overflow })]);

            return { spinId: spin.rows[0].id, payoutId, createdAt: spin.rows[0].created_at, prize: picked, credited, rewardStars };
          });

          const prize = result.prize;
          return Response.json({ ok: true, spin: { id: result.spinId, type: prize ? (result.rewardStars ? "PAID" : "FREE") : "FREE", priceStars: 0, status: "COMPLETED", createdAt: result.createdAt }, reward: {
            id: result.payoutId ?? result.spinId, kind: prize.kind, title: prize.title, subtitle: prize.subtitle, amount: Number(prize.amount) || undefined, wonAt: result.createdAt,
