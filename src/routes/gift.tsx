...
function GiftScreen() {
  ...
  const open = async () => {
    ...
    try {
      ...
      window.setTimeout(() => setReward(result), 400);
    } finally {
      window.setTimeout(() => { setOpening(false); setBusy(false); }, 450);
    }
  };

  let description = "Открой коробку — внутри случайная награда с разным шансом.";
  if (snapshot.gift.state === "COOLDOWN") description = "Подарок уже получен. Возвращайся после окончания таймера.";
