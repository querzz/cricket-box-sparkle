  ...
  server: { handlers: {
    ...
    POST: async ({ request }) => {
      try {
        ...
        const seasonId=(body.seasonId??"").trim(); const name=(body.name??"").trim(); const triggerType=body.triggerType??""; const value=body.triggerValue===null||body.triggerValue===undefined?null:Number(body.triggerValue);
    ...
    PATCH: async ({ request }) => {
      try {
        ...
        const actor=await authenticateAdmin(body.initData??""); if(!body.id||!body.seasonId||!body.action) return Response.json({ok:false,code:"INVALID_INPUT"},{status:400});
        const result=await withTransaction(async client=>{
          const row=await client.query(`SELECT id::text,status FROM season_drop_events WHERE id=$1::uuid AND season_id=$2::uuid FOR UPDATE`,[body.id,body.seasonId]);
          ...
          else if(body.action==="ACTIVATE"){ if(row.rows[0].status!=="SCHEDULED") throw new Error("DROP_NOT_SCHEDULED"); await client.query(`UPDATE season_drop_events SET trigger_type='MANUAL',trigger_value=NULL WHERE id=$1::uuid`,[body.id]); await activateDueDrops(client,body.seasonId); }
          else throw new Error("INVALID_ACTION");
          await client.query(`INSERT INTO audit_logs(admin_id,action,entity_type,entity_id,after_data) VALUES($1::uuid,$2,'season_drop',$3,NULL)`,[actor.id,`DROP_${body.action}`,body.id]);
          return true;
        });
