      ...
      try {
        ...
        const result = await withTransaction(async (client) => {
          ...
          const state = await client.query<{
            ...
            `SELECT is_participant, is_subscribed, daily_gift_claimed_at::text, stars_balance
               FROM user_state
          ...
          // The UI promises a 24-hour cooldown, so enforce exactly 24 hours server-side.
          if (current.daily_gift_claimed_at) {
            const claimedAt = new Date(current.daily_gift_claimed_at).getTime();
            if (Number.isFinite(claimedAt) && Date.now() - claimedAt < 24 * 60 * 60 * 1000) {
              throw new Error("GIFT_COOLDOWN");
            }
          }

          const credited = Math.min(GIFT_AMOUNT, Math.max(0, MAX_STARS - Number(current.stars_balance)));
          if (credited <= 0) throw new Error("GIFT_BALANCE_FULL");
          ...
          await client.query(
            ...
            [
              user.rows[0].id,
              JSON.stringify({ credited, requested: GIFT_AMOUNT, seasonId: currentSeason.id, seasonState: currentSeason.state, payoutId: payout.rows[0].id }),
            ],
          );
        ...
        return Response.json({
          ...
          reward: {
            ...
            status: "RECEIVED",
            payoutNote: result.credited < GIFT_AMOUNT
              ? `Лимит баланса: зачислено ${result.credited} из ${GIFT_AMOUNT} Stars.`
              : "Ежедневный подарок зачислен на баланс Stars.",
          },
        });
      } catch (error) {
