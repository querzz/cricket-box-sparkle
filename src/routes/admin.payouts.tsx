...
function AdminPayouts() {
  ...
  async function refresh(search = query) {
    ...
    try {
      const d = await request(
        `/api/admin/payouts?initData=${encodeURIComponent(initData())}&search=${encodeURIComponent(search)}`,
      );
      setRows(d.payouts ?? []);
      setCounts({
  ...
  async function setStatus(id: string, status: "REVIEW" | "PAID" | "FAILED" | "CANCELLED") {
    ...
    try {
      ...
      setError(
        e instanceof Error && e.message === "INVALID_TRANSITION"
          ? "Недопустимый переход статуса выплаты."
  ...
  async function bulkStatus(status: "PAID" | "FAILED" | "CANCELLED") {
    ...
    try {
      ...
      setError(
        e instanceof Error && e.message === "INVALID_TRANSITION"
          ? "Некоторые выплаты нельзя перевести в этот статус."
  ...
  const filtered = useMemo(
    () => (filter === "Все" ? rows : rows.filter((x) => x.status === filter)),
    [rows, filter],
  );
      ...
      <div className="space-y-4 pb-8">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-[10px] text-primary-glow"
          >
        ...
        <GlassCard className="px-4 py-4" glow>
          ...
          <div className="mt-4 grid grid-cols-5 gap-2">
            ...
            <Metric label="Проверка" value={String(counts.review)} />
            <Metric label="Выдано" value={String(counts.paid)} />
            <Metric label="Ошибки" value={String(counts.failed)} />
            <Metric label="Отменено" value={String(counts.cancelled)} />
          </div>
        </GlassCard>
        ...
        <GlassCard className="space-y-3 px-3 py-3">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {(["Все", "Ожидает", "На проверке", "Выдан", "Ошибка", "Отменён"] as const).map((x) => (
              <button
                key={x}
                onClick={() => setFilter(x)}
          ...
          {selectable.length > 0 && (
            <div className="flex items-center justify-between gap-2 border-t border-glass-border pt-2">
              ...
              {selected.size > 0 && (
                <div className="flex gap-1.5">
                  <button
                    ...
                    className="rounded-lg border border-primary/30 bg-primary/10 px-2.5 py-1.5 text-[10px] font-semibold"
                  >
                    <Check className="mr-1 inline size-3" /> Выдать {selected.size}
                  </button>
                  <button
                    disabled={saving !== null}
                    onClick={() => void bulkStatus("FAILED")}
                    className="rounded-lg border border-destructive/30 bg-destructive/10 px-2.5 py-1.5 text-[10px] font-semibold"
                  >
                    <XCircle className="mr-1 inline size-3" /> Ошибка
                  </button>
                  <button
                    disabled={saving !== null}
                    onClick={() => void bulkStatus("CANCELLED")}
                    className="rounded-lg border border-glass-border bg-muted/15 px-2.5 py-1.5 text-[10px] font-semibold"
                  >
                    <Ban className="mr-1 inline size-3" /> Отмена
                  </button>
                </div>
              )}
            </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2.5">
            {filtered.map((p) => (
              <GlassCard key={p.id} className="px-3.5 py-3.5">
                  ...
                  <div className="flex min-w-0 items-start gap-2">
                    <div className="pt-1">
                      {p.status === "На проверке" && (
                        <input
                          type="checkbox"
                          aria-label={`Выбрать выплату ${p.id}`}
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-sm font-semibold">{p.username}</p>
                      <p className="text-[9px] text-muted-foreground">ID {p.telegramId} · {p.time}</p>
                    </div>
                  </div>
                ...
                <div className="mt-3 rounded-xl border border-glass-border bg-muted/15 px-3 py-2.5">
                  ...
                  <div className="mt-1 flex items-end justify-between gap-2">
                    <p className="text-sm font-semibold">{p.prize}</p>
                    <p className="text-xs font-semibold text-muted-foreground">{p.amount}</p>
                  </div>
                </div>

                {(p.status === "Ожидает" || p.status === "На проверке") && (
                  <div className="mt-3 flex gap-2">
                    {p.status === "Ожидает" ? (
                      <button
                        disabled={saving !== null}
                        onClick={() => void setStatus(p.id, "REVIEW")}
                        className="flex-1 rounded-xl border border-warning/30 bg-warning/10 px-3 py-2 text-[10px] font-semibold"
                      >
                        <Clock3 className="mr-1 inline size-3.5" /> Проверить
                      </button>
                    ) : (
                      <>
                        <button
                          ...
                          className="flex-1 rounded-xl border border-primary/30 bg-primary/10 px-3 py-2 text-[10px] font-semibold"
                        >
                          <Check className="mr-1 inline size-3.5" /> Выдать
                        </button>
                        <button
                          disabled={saving !== null}
                          onClick={() => void setStatus(p.id, "FAILED")}
                          className="flex-1 rounded-xl border border-destructive/30 bg-destructive/10 px-3 py-2 text-[10px] font-semibold"
                        >
                          <XCircle className="mr-1 inline size-3.5" /> Ошибка
                        </button>
                        <button
                          disabled={saving !== null}
                          onClick={() => void setStatus(p.id, "CANCELLED")}
                          className="flex-1 rounded-xl border border-glass-border bg-muted/15 px-3 py-2 text-[10px] font-semibold"
                        >
                          <Ban className="mr-1 inline size-3.5" /> Отменить
                        </button>
                      </>
                    )}
                  </div>
            ...
            {filtered.length === 0 && (
              <GlassCard className="px-4 py-8 text-center text-xs text-muted-foreground">Выплат пока нет.</GlassCard>
            )}
          </div>
        )}

        <GlassCard className="px-4 py-3 text-[11px] leading-relaxed text-muted-foreground">
          Lifecycle: <b>Ожидает → На проверке → Выдано / Ошибка / Отменено</b>. Завершённые статусы нельзя
          открыть обратно. Для вывода Stars при ошибке или отмене средства автоматически возвращаются на
          баланс и действие попадает в аудит.
        </GlassCard>
      </div>
    </AppShell>
  );
...
function Status({ status }: { status: PayoutStatus }) {
  ...
  return (
    <span className={`inline-flex whitespace-nowrap rounded-full border px-2.5 py-1 text-[9px] font-semibold ${cls}`}>
      {status}
    </span>
  );
}
