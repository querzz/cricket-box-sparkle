...
function displaySeasonTitle(code: string, name: string) {
  ...
  const namedMatch = cleanName.match(/^CRICKET\s+BOX\s*#?(\d+)(?:[-_].*)?$/i);
  if (namedMatch) return `CRICKET BOX #${namedMatch[1]!.padStart(3, "0")}`;
    ...
    GET: async ({ request }) => {
      try {
        ...
        return Response.json({ ok:true, snapshot:{
          ...
          withdrawals:withdrawalResult.rows.map((w)=>({id:w.id,rewardTitle:"Telegram Stars",amount:Number(w.amount)||0,requestedAt:w.created_at,status:w.status==="PAID"?"PAID":["FAILED","CANCELLED"].includes(w.status)?"REJECTED":"PENDING"})),
