...
async function refundWithdrawalStars(client: PoolClient, payoutId: string, userId: string, amount: number, adminId: string, reason: Status) {
  ...
  if (credited > 0) {
    await appendStarsLedger(client, {
      userId, type: "REFUND_REVERSAL", amount: credited, balanceDelta: credited,
      referenceId: payoutId, idempotencyKey: `withdrawal-refund:${payoutId}:credited`,
      metadata: { requestedAmount: requested, creditedAmount: credited, overflowAmount: overflow, reason },
    });
  }
  if (overflow > 0) {
    await appendStarsLedger(client, {
      userId, type: "CAPPED_OVERFLOW_BURNED", amount: -overflow, balanceDelta: 0,
      referenceId: payoutId, idempotencyKey: `withdrawal-refund:${payoutId}:overflow`,
      metadata: { requestedAmount: requested, creditedAmount: credited, overflowAmount: overflow, reason },
    });
  }
  await client.query(
    ...
    [adminId, payoutId, JSON.stringify({ requestedAmount: requested, creditedAmount: credited, overflowAmount: overflow, reason })],
  );
}
...
async function changePayout(client: PoolClient, adminId: string, id: string, nextStatus: Status) {
  ...
  await client.query(
    `UPDATE payouts SET status=$2, operator_admin_id=$3::uuid,
       paid_at=CASE WHEN $2='PAID' THEN COALESCE(paid_at, now()) ELSE paid_at END, updated_at=now()
     WHERE id=$1::uuid`, [id, nextStatus, adminId],
  );
  ...
  if (isWithdrawal && ["FAILED", "CANCELLED"].includes(nextStatus) && ["PENDING", "REVIEW"].includes(before.status)) {
    await refundWithdrawalStars(client, id, before.user_id, Number(before.amount), adminId, nextStatus);
  }
  await client.query(
...
export const Route = createFileRoute("/api/admin/payouts")({ server: { handlers: {
  GET: async ({ request }) => {
    try {
      ...
      const result = await query<PayoutRow>(`SELECT py.id::text, py.created_at::text, u.telegram_id::text, u.username,
        py.kind, py.amount::text, py.currency, py.status, py.note, p.title AS prize_title, p.subtitle AS prize_subtitle
        FROM payouts py JOIN users u ON u.id=py.user_id LEFT JOIN prizes p ON p.id=py.prize_id
        WHERE ($2='' OR py.id::text ILIKE $1 OR u.telegram_id::text ILIKE $1 OR COALESCE(u.username,'') ILIKE $1 OR COALESCE(p.title,'') ILIKE $1)
          AND ($3='' OR py.status=$3) ORDER BY py.created_at DESC LIMIT 200`, [pattern, search, status]);
      const counts = await query<{ pending:string; review:string; paid:string; failed:string; cancelled:string }>(`SELECT
        COUNT(*) FILTER (WHERE status='PENDING')::text AS pending, COUNT(*) FILTER (WHERE status='REVIEW')::text AS review,
        COUNT(*) FILTER (WHERE status='PAID')::text AS paid, COUNT(*) FILTER (WHERE status='FAILED')::text AS failed,
        COUNT(*) FILTER (WHERE status='CANCELLED')::text AS cancelled FROM payouts`);
      return Response.json({ ok:true, counts:{ pending:Number(counts.rows[0]?.pending??0), review:Number(counts.rows[0]?.review??0), paid:Number(counts.rows[0]?.paid??0), failed:Number(counts.rows[0]?.failed??0), cancelled:Number(counts.rows[0]?.cancelled??0) }, payouts:result.rows.map(row=>({
        ...
        type:row.kind==="STARS"?"Stars":row.kind==="PREMIUM"?"Premium":"Деньги",amount:row.kind==="STARS"?`${row.amount} ⭐`:`${row.amount} ${row.currency??""}`.trim(),
  ...
  PATCH: async ({ request }) => {
    try { const body=await request.json() as {initData?:unknown;id?:unknown;status?:unknown}; const admin=await authenticateAdmin(typeof body.initData==="string"?body.initData:"");
      const id=typeof body.id==="string"?body.id:""; const nextStatus=typeof body.status==="string"?body.status as Status:"" as Status;
      ...
      await withTransaction(client=>changePayout(client,admin.id,id,nextStatus)); return Response.json({ok:true});
  ...
  POST: async ({ request }) => {
    try { const body=await request.json() as {initData?:unknown;ids?:unknown;status?:unknown}; const admin=await authenticateAdmin(typeof body.initData==="string"?body.initData:"");
      const ids=Array.isArray(body.ids)?body.ids.filter((id):id is string=>typeof id==="string"):[]; const nextStatus=typeof body.status==="string"?body.status as Status:"" as Status;
      ...
      const uniqueIds=[...new Set(ids)]; await withTransaction(async client=>{for(const id of uniqueIds) await changePayout(client,admin.id,id,nextStatus);}); return Response.json({ok:true,count:uniqueIds.length});
