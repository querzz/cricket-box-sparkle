  ...
  server: { handlers: {
    POST: async ({ request }) => {
      try {
        ...
        const seasonResult = await query<{ starts_at: string | null; ends_at: string | null; paid_spin_price: number }>(
          `SELECT starts_at::text,ends_at::text,paid_spin_price FROM seasons WHERE id=$1::uuid`,
          [seasonId],
        );
        ...
        const prizes = await query<{
          ...
          `SELECT id::text,kind,title,quantity_total,quantity_remaining,amount::text,unit_cost::text,metadata
             FROM prizes
            WHERE season_id=$1::uuid AND is_active=TRUE AND quantity_remaining>0
        ...
        const guardrails = evaluateEconomyGuardrails({
          ...
          prizes: simulationPrizes.map(prize => ({
            id: prize.id,
            kind: prize.kind,
            title: prize.title,
            amount: prize.amount,
            unitCost: prize.unitCost,
