...
export const cricketApi = {
  async getSession(): Promise<ServiceResult<SessionSnapshot>> {
    if (inTelegram()) return backendSession();
  ...
  async spin(options: SpinOptions = {}): Promise<ServiceResult<{ reward: Reward; snapshot: SessionSnapshot }>> {
    ...
    const inventory = state.prizes.find((p) => p.id === picked.id); if (!inventory) return fail("NO_ATTEMPTS", "Приз недоступен."); inventory.remaining -= 1;
    ...
    state.spin.totalSpins += 1; state.rewards = [reward, ...state.rewards]; localPersist(state);
  ...
  async setSubscribed(value: boolean): Promise<ServiceResult<SessionSnapshot>> { state.user.isSubscribed = value; localPersist(state); return ok(structuredClone(state)); },
  async setStarsAmount(amount: number): Promise<ServiceResult<SessionSnapshot>> { state.stars.amount = Math.max(0, Math.min(state.stars.max, Math.round(amount))); localPersist(state); return ok(structuredClone(state)); },
  ...
  async resetDailyFreeSpin(): Promise<ServiceResult<SessionSnapshot>> { state.spin.freeSpins = 1; localPersist(state); return ok(structuredClone(state)); },
