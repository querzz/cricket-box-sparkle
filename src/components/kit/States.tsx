...
export function EmptyState({ title, description, icon, action }: { title: string; description?: string; icon?: ReactNode; action?: ReactNode }) {
  return (
    <GlassCard className="flex flex-col items-center gap-3 px-6 py-10 text-center">
      <div className="grid size-12 place-items-center rounded-full bg-primary/12 text-primary">{icon ?? <Inbox className="size-5" />}</div>
      <div>
...
export function ErrorState({
  ...
  return (
    <GlassCard className="flex flex-col items-center gap-3 px-6 py-10 text-center">
      <div className="grid size-12 place-items-center rounded-full bg-destructive/15 text-destructive"><WifiOff className="size-5" /></div>
      <div>
        <h3 className="font-display text-sm uppercase tracking-[0.16em]">{title}</h3>
...
export function NoticeBar({ tone = "info", children, className }: { tone?: "info" | "warning" | "danger"; children: ReactNode; className?: string }) {
  return <div className={cn("rounded-xl border px-3.5 py-2.5 text-[11px] leading-relaxed", tone === "info" && "border-glass-border bg-muted/40 text-muted-foreground", tone === "warning" && "border-warning/35 bg-warning/10 text-warning", tone === "danger" && "border-destructive/40 bg-destructive/10 text-destructive", className)}>{children}</div>;
}
